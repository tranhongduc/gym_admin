...
const CreateUser = () => {
  ...
  const dummyRequest = ({ file, onSuccess, onError }) => {
    ...
    if (file.originFileObj) {
      // Nếu file hợp lệ, thực hiện các thao tác kiểm tra tại đây (không cần gửi API)
      setTimeout(() => {
        console.log("File is ready to upload:", file);
        onSuccess("ok"); // Gọi onSuccess nếu file hợp lệ
      }, 1000);
    } else {
      onError("File is not valid"); // Gọi onError nếu file không hợp lệ
    }
  };
  ...
  const onFinish = async (values) => {
    ...
    try {
      if (file && file.originFileObj) {
        ...
        const formData = new FormData();
        formData.append("file", file.originFileObj); // Sử dụng file.originFileObj để append vào FormData
        ...
        const values = { avatar: res.mediaUrl }; // Thuộc tính mới
        const updatedUsera = { ...userData, ...values };
        setUserData(updatedUsera);
        console.log("userData", userData);
        const registerResponse = await user_register(userData); // Gọi API đăng ký tài khoản
        console.log("User Registration Response:", registerResponse);
        navigate("/app/user");
        ...
        >
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            <Upload
              ...
              customRequest={dummyRequest} // Gọi hàm dummyRequest để kiểm tra trước khi upload
